...
object FirstQuarter {
  ...
  def parseClock(field: String): Double = {
    ...
    if (split.length > 1) {
      return split(0).toDouble + split(1).toDouble / 60.0
    } else {
  ...
  def main(args: Array[String]) {
    ...
    println("Top Ten First-Quarter Outside Shooters (by points per shot from 5+ ft out):")

    shots.filter(shot => shot.shotDistance > 5.0).filter(_.gameClock < 12.0).
      groupBy($"player").agg(count("shotClock"), sum("playPoints")).flatMap(group => {
